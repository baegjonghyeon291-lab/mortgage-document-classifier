...
<style>
:root{{--ink:#24143d;--muted:#746987;--line:#e8def4;--bg:#f8f5fc;--panel:#fff;--purple:#7137a8;--purple-dark:#4d2375;--purple-soft:#f1e9f9}}
*{{box-sizing:border-box}} body{{margin:0;background:radial-gradient(circle at top right,#eadcf7 0,transparent 34%),var(--bg);color:var(--ink);font:14px/1.5 Inter,Segoe UI,sans-serif}}
...
h1{{margin:0;font-size:30px;letter-spacing:-.03em;color:var(--purple-dark)}} .eyebrow{{color:var(--purple);font-weight:700;text-transform:uppercase;letter-spacing:.12em;font-size:11px}}
.subtitle{{color:var(--muted);margin:8px 0 0}} .metrics{{display:grid;grid-template-columns:repeat(5,1fr);gap:12px;margin:28px 0}}
.metric,.panel{{background:var(--panel);border:1px solid var(--line);border-radius:16px;box-shadow:0 10px 30px #4d23750d}}
.metric{{padding:18px;border-top:3px solid #b98cdb}} .metric span{{display:block;color:var(--muted);font-size:11px;font-weight:700}} .metric strong{{font-size:30px}}
...
select,label{{border:1px solid var(--line);border-radius:9px;background:white;padding:8px 10px}} select:focus{{outline:2px solid #c6a4e2}} label{{cursor:pointer}}
.table-wrap{{overflow:auto;max-height:540px}} table{{border-collapse:collapse;width:100%;min-width:980px}} th{{position:sticky;top:0;background:var(--purple-soft);text-align:left;color:var(--purple-dark);font-size:11px;text-transform:uppercase;letter-spacing:.05em}}
th,td{{border-bottom:1px solid var(--line);padding:11px 10px;vertical-align:top}} tbody tr:hover{{background:#faf7fd}}
