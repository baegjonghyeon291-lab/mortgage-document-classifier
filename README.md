...
`Page N of M`을 가장 강한 경계 증거로 사용합니다. 같은 유형과 총 페이지 수 안에서 동일한
페이지 번호가 반복되면 별도 문서 인스턴스로 분리합니다. 서로 경쟁하는 문서 후보가 여러 개인데
식별 근거가 없는 페이지는 임의로 합치지 않고 별도 그룹으로 남깁니다. 총 페이지 수가 명시되지
않으면 문서가 완전하다고 단정하지 않습니다.

package_02에서는 다음과 같이 **9개 논리 문서 그룹**을 복원했습니다.

| 유형 | 그룹 수 | 비고 |
|---|---:|---|
| `URLA_1003` | 1 | 명시적 페이지 번호가 없어 유형 단위 보수적 그룹핑 |
| `INCOME_DOC` | 3 | 2페이지 W-2 문서 2개와 페이지 번호 없는 요약 1개 |
| `CREDIT_REPORT` | 1 | `Page 1 of 9`부터 `Page 9 of 9` 및 부속 페이지 |
| `TITLE_REPORT` | 4 | 5페이지 문서 2개, 4페이지 문서 1개, Chain of Title 1개 |

4페이지 `TITLE_REPORT` 그룹은 1페이지가 발견되지 않아 불완전 문서로 표시합니다. 같은 그룹
안에 중복된 문서 페이지 번호는 없습니다.
